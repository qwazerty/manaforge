/**
 * UI configuration constants shared across Svelte components.
 * Mirrors the legacy UIConfig class but as a typed ESM export.
 */
export const UIConfig = {
    ...
    CSS_CLASSES: {
        button: {
            ...
            primarySmall: 'flex-1 bg-amber-500/30 hover:bg-amber-500/50 border border-amber-400/80 hover:border-amber-300 text-amber-100 hover:text-white py-2 px-2 rounded-lg font-semibold transition-all duration-200',
    ...
    getPhaseDisplayName(phaseId: string) {
        const phase = UIConfig.GAME_PHASES.find((p) => p.id === phaseId);
