...
class ConnectionManager:
    ...
    def disconnect(self, websocket: WebSocket, game_id: str):
        ...
        if game_id in self.active_connections:
            self.active_connections[game_id] = [
                (ws, pid, ping)
                for ws, pid, ping in self.active_connections[game_id]
                if ws != websocket
            ]
    ...
    async def broadcast_to_game(
        self, game_id: str, message: dict, exclude_websocket: Optional[WebSocket] = None
    ):
        ...
        while True:
            ...
            if message.get("type") == "ping":
                ...
                try:
                    ...
                    current_phase = getattr(
                        updated_game_state.phase, "value", updated_game_state.phase
                    )
                    if current_phase:
                        broadcast_info["action_result"].setdefault(
                            "phase", current_phase
            ...
            # Handle random animations (coin flip, dice rolls)
                ...
                if animation_type == "coin":
                    ...
                    if result == 20:
                        result_message = "🎯 Rolled 20 - CRITICAL!"
                    elif result == 1:
                        result_message = "🎯 Rolled 1 - CRITICAL FAIL!"
                    else:
                        result_message = f"🎯 Rolled {result}"
                else:
                    result = None
            ...
            # Handle draft-specific messages
                ...
                if room:
                    if message.get("type") == "add_bot":
                        ...
                        await manager.broadcast_to_game(
                            game_id, {"type": "draft_starting"}
                        )
                        await engine.start_draft(game_id)
                    elif message.get("type") == "pick_card":
                        engine.pick_card(
                            game_id, player_id, message.get("card_unique_id")
                        )
                        ...
                        if player:
                            ...
                            await websocket.send_text(
                                json.dumps(
                                    {"type": "decklist_data", "decklist": decklist}
                                )
                            )
