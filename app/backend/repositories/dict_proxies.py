...
from typing import Any, Dict, Iterator, List, Optional, TypeVar, Generic, cast
    ...
    def __setitem__(self, key: str, value: T) -> None:
        ...
        with connect() as conn:
            with conn.cursor() as cur:
                ...
                """).format(
                    self._sql_table(),
                    self._sql_id_col(), self._sql_data_col(),
                    self._sql_id_col(),
                    self._sql_data_col(), self._sql_data_col()
                )
...
class ReplaysProxy:
    ...
    def __contains__(self, game_id: str) -> bool:
        with connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT 1 FROM game_replays WHERE game_id = %s LIMIT 1",
                    (game_id,)
                )
                return cur.fetchone() is not None
    ...
    def get(self, game_id: str, default: Optional[List] = None) -> Optional[List[Dict[str, Any]]]:
        try:
            return self[game_id]
        except KeyError:
    ...
    def __delitem__(self, game_id: str) -> None:
        with connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "DELETE FROM game_replays WHERE game_id = %s",
                    (game_id,)
                )
            conn.commit()
...
class ActionHistoryProxy:
    ...
    def append(self, game_id: str, entry: Dict[str, Any], max_entries: int = DEFAULT_ACTION_HISTORY_LIMIT) -> None:
    ...
    def get_recent(self, game_id: str, limit: int = DEFAULT_ACTION_HISTORY_LIMIT) -> List[Dict[str, Any]]:
        with connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
...
class ChatMessagesProxy:
    ...
    def append(self, game_id: str, entry: Dict[str, Any], max_entries: int = DEFAULT_CHAT_MESSAGES_LIMIT) -> None:
    ...
    def get_recent(self, game_id: str, limit: int = DEFAULT_CHAT_MESSAGES_LIMIT) -> List[Dict[str, Any]]:
        with connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    """
...
class PendingDecksProxy:
    ...
    def __contains__(self, game_id: str) -> bool:
        with connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT 1 FROM pending_decks WHERE game_id = %s LIMIT 1",
                    (game_id,)
                )
                return cur.fetchone() is not None
    ...
    def __delitem__(self, game_id: str) -> None:
        with connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "DELETE FROM pending_decks WHERE game_id = %s",
                    (game_id,)
                )
            conn.commit()
...
class CubePoolsProxy:
    ...
    def __getitem__(self, room_id: str) -> List[str]:
        with connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT cube_pool FROM draft_rooms WHERE id = %s",
                    (room_id,)
                )
                row = cur.fetchone()
...
class CubePoolCursorsProxy:
    ...
    def __getitem__(self, room_id: str) -> int:
        with connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT pool_cursor FROM draft_rooms WHERE id = %s",
                    (room_id,)
                )
                row = cur.fetchone()
                if row is None:
...
class CubeCardCacheProxy:
    ...
    def __getitem__(self, room_id: str) -> Dict[str, Dict[str, Any]]:
        with connect() as conn:
            with conn.cursor() as cur:
                cur.execute(
                    "SELECT cube_cache FROM draft_rooms WHERE id = %s",
                    (room_id,)
                )
                row = cur.fetchone()
                if row is None:
    ...
    def get(self, room_id: str, default: Optional[Dict] = None) -> Dict[str, Dict[str, Any]]:
        try:
            return self[room_id]
        except KeyError:
            return default if default is not None else {}
